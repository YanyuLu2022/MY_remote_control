...
void LCD_ShowChar(uint16_t x, uint16_t y, uint8_t num, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
	...
	for (i = TypefaceNum - 1; i < TypefaceNum; i--) // 字节倒序
	{
		if (sizey == 12)
		...
		for (t = 7; t < 8; t--) // 位倒序
		{
			...
			{
				if (temp & (0x01 << t))
					LCD_WR_DATA(fc); // 显示前景色
				else
					LCD_WR_DATA(bc); // 显示背景色
			}
			else // 叠加模式
			{
				if (temp & (0x01 << t))
...
void LCD_ShowString(uint16_t x, uint16_t y, const uint8_t *p, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
	...
	while (end >= p)
	{
		LCD_ShowChar(x, y, *end, fc, bc, sizey, mode);
		x += sizex; // 每次向右移动一个字符宽度
		end--;
	}
}
/******************************************************************************
...
void LCD_ShowChinese16x16(uint16_t x, uint16_t y, uint8_t s, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
	...
	for (k = 0; k < HZnum; k++)
	{
		if ((tfont16[k].Index[0] == s)) // 查找到目标汉字
		{
			LCD_Address_Set(x, y, x + sizey - 1, y + sizey - 1);

			// 倒序读取点阵数据，从最后一行开始
			for (i = TypefaceNum - 1; i < TypefaceNum; i--)
			{
				for (j = 7; j <= 7; j--)
				{
					if (!mode) // 非叠加方式
...
void LCD_ShowChinese32x32(uint16_t x, uint16_t y, uint8_t s, uint16_t fc, uint16_t bc, uint8_t sizey, uint8_t mode)
{
	...
	for (k = 0; k < HZnum; k++)
	{
		if ((tfont32[k].Index[0] == s)) // 查找到目标汉字
		{
			LCD_Address_Set(x, y, x + sizey - 1, y + sizey - 1);
